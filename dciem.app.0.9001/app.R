...
# Generate Time Choices
generate_time_choices <- function(start_time = "07:00") {
  full_sequence <- format(seq(as.POSIXct("00:00", format="%H:%M"),
                              as.POSIXct("23:59", format="%H:%M"),
                              by="min"), "%H:%M")
  start_index <- match(start_time, full_sequence)
...
server <- function(input, output, session) {
  ...
  # set observers for timeIn and timeOut,
  ...
  observeEvent(input$addBtn, {
    values$timeOutChanged <- FALSE # Reset the flag indicating manual change of timeOut


    if (nrow(rv$data) > 0) { # Check if there is at least one row in the data
      lastTimeOut <- rv$data$TimeOut[nrow(rv$data)] # Fetch the TimeOut of the last row
      updateSelectInput(session, "timeIn", selected = lastTimeOut) # Update TimeIn to be the same as the last row's TimeOut
      dateTimeIn <- ymd_hm(paste(input$date, lastTimeOut)) # Recalculate dateTimeIn with the updated TimeIn
    } else {
      dateTimeIn <- ymd_hm(paste(input$date, input$timeIn)) # If there are no rows, use the current input for TimeIn
    }

    dateTimeIn <- ymd_hm(paste(input$date, input$timeIn)) # Combine Date and BT and convert to POSIXct
    dateTimeOut <- ymd_hm(paste(input$date, input$timeIn))

    #--------calculate Surface Interval (SI) ---------
    if (nrow(rv$data) > 0) {
      lastDateTimeOut <- ymd_hm(paste(rv$data$Date[nrow(rv$data)], rv$data$TimeOut[nrow(rv$data)]))
    ...
    RG2val <- if (nrow(rv$data) >= 1) {
      ...
      if (nrow(rv$data) == 1) {
        ...
        # For subsequent rows, increment based on the last RG2
        increment_letter(rv$data$RG2[nrow(rv$data)])
      }
    } else {
      ""
    }
    ...
    updateSelectInput(session, "timeIn", selected = newEntry$TimeOut)
  })


  #-------- Draft function to allow cells to be edited (needs refining) --------
  ...
  output$downloadExcel <- downloadHandler(
    ...
    content = function(file) {
      ...
      addWorksheet(wb, "Data") # Add a sheet named "Data"
      writeData(wb, "Data", rv$data) # Write the data frame to the "Data" sheet
      saveWorkbook(wb, file, overwrite = TRUE) # Save the workbook
    }
  )
